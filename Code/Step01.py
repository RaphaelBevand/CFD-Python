"""
Solve the one dimensional linear convection.
"""
import numpy as np

from Code import Simulation
from Code import Solver
from Solvers import step_01 # @UnresolvedImport
...
class LinearConvection(Solver):
    ...
    @property
    def label(self):
        return f"Linear Convection (Courant={self.courant:.3f})"
    
    def solve_timestep(self):
        step_01(dx = self.dx, dt = self.dt, u = self.y)
...
if __name__ == "__main__":
    ...
    simulation.add_solver(LinearConvection(courant=0.1, **kwargs))
    simulation.add_solver(LinearConvection(courant=0.5, **kwargs))
    simulation.add_solver(LinearConvection(courant=0.9, **kwargs))
