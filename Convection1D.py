...
def set_initial_condition(initial, nx, xmax, tmax, cfl, ratio):
...
class LinearConvection(Solver):
    ...
    @property
    def label(self):
        return f"Linear Convection (Speed={self.speed:.3f})"
...
class Diffusion(Solver):
    ...
    @property
    def label(self):
        return f"Diffusion (nu={self.nu:.5f})"
...
class ConvectionDiffusion(Solver):
    ...
    @property
    def label(self):
        return f"Convection-Diffusion (nu={self.nu:.5f})"
...
x, t, u, nu = set_initial_condition(
    initial = initial, 
    nx = 901, 
    xmax = 6.0, 
    tmax = 3.0,
